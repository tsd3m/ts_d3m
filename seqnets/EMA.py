...
class MultiHeadEMA(nn.Module):
    ...
    def __init__(
        ...
        self.gamma = nn.Parameter(torch.Tensor(kernel_dim, ndim)) # 对应原文中的eta
    ...
    def _calc_coeffs(self):
        self._coeffs = None
        # D x N x 1
        p = torch.sigmoid(self.delta) # 对应的是alpha
        alpha = torch.sigmoid(self.alpha) # 对应的是delta
        q = 1.0 - p * alpha # 对应的是\phi
        return p, q
